...
public class VirtualKeypad extends View
{
	...
	public void resize(int w, int h)
	{
		...
		transparency = prefs.getInt("vkeypadTransparency", 55);
	}
	...
	private void makeTopBottom(int w, int h)
	{
		...
		x = (w + leftShoulder.getWidth() - buttons.getWidth() -
	...
	private int getDpadStates(float x, float y)
	{
		...
		else if (x > cx + dpadDeadZone) states |= Emulator.GAMEPAD_RIGHT;
		if (y < cy - dpadDeadZone)
			states |= Emulator.GAMEPAD_UP;
		else if (y > cy + dpadDeadZone) states |= Emulator.GAMEPAD_DOWN;
	...
	private static final int[] EXTRA_BUTTONS = { Emulator.GAMEPAD_B_TURBO,
